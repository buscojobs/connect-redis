...
module.exports = function(session){
  ...
  RedisStore.prototype.get = function(sid, fn){
    sid = this.prefix + sid;
    debug('GET "%s"', sid);
    this.client.hgetall(sid, function(err, data){
  ...
  RedisStore.prototype.set = function(sid, sess, fn){
    ...
    try {
      ...
      var aspSess = sess.asp;
      delete sess.asp;
      sess = JSON.stringify([JSON.stringify(sess), aspSess]);
      ttl = ttl || ('number' == typeof maxAge
          ? maxAge / 1000 | 0
          : oneDay);

      debug('SETEX "%s" %s %s', sid, "data", sess);
      var client = this.client;
      client.hset(sid, "data", sess, function(err){
        err || debug('SETEX complete');
        debug('EXPIRE "%s" %s', sid, ttl, sess);
        client.expire(sid, ttl, function(err) {
